...
utils.checkParameters = (dati, requiredFields) => {
  ...
  for (let i = 0; i < requiredFields.length; i += 1) {
    if (!(requiredFields[i] in dati) || dati[requiredFields[i]] === undefined
        || dati[requiredFields[i]] === null) {
      return false;
    }
  }
 ...
 */
utils.sha256 = (pwd) => crypto.createHash('sha256').update(pwd).digest('base64');
 ...
 */
utils.convertToNormalObject = (object) => JSON.parse(JSON.stringify(object));

 ...
 * @property {Function} getByConversazione - Ottiene tutti i messaggi
 * di una conversazione dal database.
 * @property {Function} getByConversazionePaginated - Ottiene i messaggi
 * paginati di una conversazione dal database.
 ...
 * @property {number} idMessaggio - Identificativo univoco del
 * messaggio (autoincrementato).
 * @property {number} fkConversazione - Chiave esterna che collega il
 * messaggio a una conversazione.
 * @property {string} corpo - Contenuto del messaggio.
 * @property {string} dataInvio - Data e ora di invio del messaggio
 * (formattata secondo il fuso orario corrente).
 ...
 * @return {Promise<Messaggio>} Promise che restituisce il
 * messaggio trovato o null se non è stato trovato
 */
messaggio.getById = (idMessaggio) => Messaggio.findByPk(idMessaggio);

/**
 * Ottiene tutti i messaggi di una conversazione ordinati per data di invio
 * @function
 * @param {Number} idConversazione - ID della conversazione
 * per cui ottenere i messaggi
 * @return {Promise<Array>} Un array di oggetti rappresentanti
 * i messaggi ordinati per data
 */
messaggio.getByConversazione = (idConversazione) => Messaggio.findAll({
  where: {
...
/**
 * Restituisce i messaggi di una conversazione paginati e
 ...
 *      pageSize: Number,
 *      currentPage: Number
 *      }>} - un oggetto contenente informazioni sulla paginazione e
 *      i messaggi della pagina richiesta
 */
 ...
 * @param {Number} dati.fkConversazione - L'ID della conversazione
 * a cui appartiene il messaggio.
 * @param {String} dati.corpo - Il corpo del messaggio.
 * @param {Boolean} dati.isUtente - Indica se il mittente del
 * messaggio è un utente.
 * @returns {Promise<Object>} Un oggetto rappresentante il
 * messaggio appena salvato.
 */
messaggio.createMessaggio = (dati) => Messaggio.create({

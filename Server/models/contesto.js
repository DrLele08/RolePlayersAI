 ...
 * @returns {Promise<Contesto | null>} - Promise che si risolve con l`istanza
 * di Contesto corrispondente all ID, o null se non trovato
 */
contesto.getContestoById = async (idContesto) => Contesto.findByPk(idContesto);

/**
 * Restituisce tutti contesti creati nel DB
 *
 * @function
 * @returns {Promise<Array<Contesto>>} - Promise che si risolve con un array
 * di istanze, oppure un array vuoto se non sono presenti
 */
contesto.getAll = async () => Contesto.findAll();
...
contesto.createContesto = async (dati) => Contesto.create({
 ...
 */
contesto.deleteContesto = async (idContesto) => Contesto.destroy({
  where: {
    idContesto,
  },
});
 ...
 * @param {String} [filters.nome] - Nome del contesto da cercare
 ...
 *      informazioni sulla paginazione e la lista di contesti
 */
contesto.getByFilter = async (filters, page, dati) => {
  ...
  if (dati.idRuolo === 1) {
    const contestiPubblici = result.filter((c) => c.isPubblico === true);
